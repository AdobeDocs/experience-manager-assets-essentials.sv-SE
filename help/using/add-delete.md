---
title: Överföra resurser till databasen
...
Om du vill lägga till nya resurser att arbeta med överför du några resurser från ditt lokala filsystem. <!-- TBD: Many of the [common file formats are supported](/help/using/supported-file-formats.md). -->

Du kan använda följande metoder för att överföra en eller flera resurser eller en mapp som innehåller resurser:

* Dra mediefiler eller mappar i användargränssnittet och följ anvisningarna på skärmen.
...
Du kan använda någon av dessa metoder för att överföra mediefiler efter att du har skapat en mapp. Om du vill skapa en tom mapp klickar du på **[!UICONTROL Create Folder]** i verktygsfältet. [!DNL Assets Essentials] erbjuder en kraftfull textsökningsfunktion, men du kan också använda mappar för att ordna dina mediefiler bättre.

När du har valt filer visas en bekräftelsedialogruta där du kan lägga till fler filer eller ta bort filer som redan är markerade. Om du vill lägga till fler filer i en markering klickar du på **[!UICONTROL Browse]** och väljer **[!UICONTROL Browse files]** eller **[!UICONTROL Browse folders]**. Lägg till fler filer eller mappar från samma eller från en annan mapp.

Klicka på **[!UICONTROL Upload]** när alla filer står i kö.
...
>[!TIP]
>
>Om du överför en mappstruktur till Assets Essentials behöver du inte skapa en .ZIP-fil med mappstrukturen, du kan överföra mappstrukturer direkt. En .ZIP-fil som överförs till Assets Essentials lagras som en ZIP-resurs och extraheras inte automatiskt efter överföringen.

## Visa överföringsförlopp och status {#upload-progress}

När du överför många mediefiler eller kapslade mappar till [!DNL Assets Essentials] kan det hända att vissa mediefiler inte kan överföras på grund av olika orsaker, t.ex. problem med duplicerade mediefiler och nätverk.

Klicka på alternativet **[!UICONTROL Upload Progress]** i verktygsfältet för att spåra överföringsförloppet. En panel visar överföringens förlopp för alla resurser.

Använd filtret i sidopanelen **[!UICONTROL Upload Progress]** om du vill visa en delmängd av resurser baserat på överföringsförloppet eller -statusen. De olika filtren ska visa alla mediefiler, slutförda överföringar, pågående överföringar, köade mediefiler som ska överföras, pausade överföringar, duplicerade mediefiler och mediefiler som inte kunde överföras.
...
* [Titta på en video om att överföra resurser i Assets Essentials](https://experienceleague.adobe.com/docs/experience-manager-learn/assets-essentials/basics/creating.html?lang=sv-SE)
