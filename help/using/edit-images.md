...
[!DNL Assets Essentials] innehåller användarvänliga redigeringsalternativ som bygger på [!DNL Adobe Photoshop Express]. De redigeringsåtgärder som är tillgängliga är Punktlagning, Beskär och räta upp, Ändra storlek på bild och Justera bild.

När du har redigerat en bild kan du spara den nya bilden som en ny version. Versionshantering hjälper dig att vid behov återställa den ursprungliga resursen senare. Om du vill redigera en bild [öppna förhandsgranskningen](/help/using/navigate-view.md#preview-assets) och klicka **[!UICONTROL Edit Image]** ![redigeringsikon](assets/do-not-localize/edit-icon.png) från järnvägen till höger.
